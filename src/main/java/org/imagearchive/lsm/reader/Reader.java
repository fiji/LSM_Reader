...
public class Reader {
		...
		final boolean verbose, final boolean thumb)
	{
		...
		try {
			...
			else IJ.error("Not a valid lsm file");
		}
	...
	private long
		getTagCount(final RandomAccessStream stream, final long position)
	{
		long tags = 0;
		try {
	...
	private ImageDirectory readImageDirectoy(final RandomAccessStream stream,
		final long startPosition, final boolean thumb)
	{
		...
		if (imDir.TIF_STRIPOFFSETS_LENGTH == 1) imDir.TIF_STRIPOFFSETS[0] =
			stripOffset;
		else imDir.TIF_STRIPOFFSETS =
		...
		if (imDir.TIF_STRIPBYTECOUNTS_LENGTH == 1) imDir.TIF_STRIPBYTECOUNTS[0] =
			stripByteOffset;
		else imDir.TIF_STRIPBYTECOUNTS =
	...
	private byte[] readTag(final RandomAccessStream stream, final int position) {
	...
	private long[] getIntTable(final RandomAccessStream stream,
		final long position, final int count)
	{
	...
	public ImagePlus open(final RandomAccessStream stream,
		final LSMFileInfo lsmFi, final boolean verbose, final boolean thumb)
	{
		final ImageDirectory firstImDir =
			(ImageDirectory) lsmFi.imageDirectories.get(0);
		if (firstImDir == null) {
			if (verbose) IJ.error("LSM ImageDir null.");
			return null;
		...
		switch (cz.ScanType) {
			...
			case 10:
				...
				return imps;
			default:
				if (verbose) IJ.error("Unsupported LSM scantype: " + cz.ScanType);
				break;
		}
		return imps;
	}
	...
	private ImagePlus readStack(final RandomAccessStream stream,
		final LSMFileInfo lsmFi, final CZLSMInfo cz, final boolean thumb)
	{
		...
		if (datatype == 0) datatype = cz.OffsetChannelDataTypesValues[0];
		switch (datatype) {
			case 1:
		...
		if (lsmFi.fileType == FileInfo.COLOR8 && lsmFi.lutSize > 0) cm =
			new IndexColorModel(8, lsmFi.lutSize, lsmFi.reds, lsmFi.greens,
				lsmFi.blues);
		else cm = LookUpTable.createGrayscaleColorModel(lsmFi.whiteIsZero);

		if (!thumb) st =
			new ImageStack((int) firstImDir.TIF_IMAGEWIDTH,
				(int) firstImDir.TIF_IMAGELENGTH, cm);
		else st = new ImageStack((int) cz.ThumbnailX, (int) cz.ThumbnailY, cm);
		...
		for (int imageCounter = 0; imageCounter < lsmFi.imageDirectories.size(); imageCounter++)
		{
			...
			for (int i = 0; i < imDir.TIF_STRIPBYTECOUNTS.length; i++)
				...
				if (imDir.TIF_COMPRESSION == 5) {
					...
					if (imDir.TIF_PREDICTOR == 2) lsmFi.compression =
						FileInfo.LZW_WITH_DIFFERENCING;
				}
				else lsmFi.compression = 0;
		...
		for (int channel = 0; channel < (int) cz.DimensionChannels; channel++) {
			...
			if (r == 0 && g == 0 && b == 0) color[1] = Color.white;
